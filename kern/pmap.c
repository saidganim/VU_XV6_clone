...
void page_init(void)
{
	...
	for (i = 1; i < npages; i++) {
		...
		// cprintf("KERN AREA: %d, %d\n", PGNUM(EXTPHYSMEM) , PGNUM(boot_alloc(0) - KERNBASE));
...
struct page_info *page_alloc(int alloc_flags)
{
	...
	struct page_info **prev = &page_free_list;
	if(!result){
		// Don't have free page;
		return NULL;
	}
	if(alloc_flags & ALLOC_HUGE){
		struct page_info *tmp = result;
		while(result){
			int i = 0;
			tmp = result;
			for(; i < 1024; ++i){
			...
			if(i == 1024){
				*prev = tmp->pp_link;
				tmp->pp_link = NULL;
				cprintf("ALLOCATED HUGE FROM %p, TO %p\n", result, tmp + PGSIZE);
				return result;
			}
			prev = &(result->pp_link);
...
{
  // Keeps page_free_list in sorted statement
	struct page_info* tmp = page_free_list;
	while(1){
		int correct_place = (!tmp) | (!(tmp->pp_link))  | (page2pa(pp) > page2pa(tmp)) |
					(page2pa(pp) > page2pa(tmp->pp_link) && page2pa(pp) < page2pa(tmp));
		if(correct_place){
			if(page2pa(pp) > page2pa(tmp)){
				cprintf("GATCHYA   pp = %p :::  tmp = %p\n", page2pa(pp), page2pa(tmp));
				if(pp->pp_link){
					while(pp->pp_link)
						pp = pp->pp_link;
					pp->pp_link = tmp;
					...
					pp->pp_link = tmp;
				page_free_list = pp;
			} else if(tmp->pp_link && tmp){
				if(pp->pp_link)
					pp[1024].pp_link = tmp->pp_link;
				else
			...
			cprintf("PAGE IS DEALLOCATED %d, to %d\n", pp - pages, tmp - pages);
			break;
		}
		tmp = tmp->pp_link;
	}
...
static void check_page_alloc(void)
{
    ...
    /* test allocation of huge page */
		...
		for (pp = page_free_list, nfree = 0; pp; pp = pp->pp_link)
				++nfree;
		cprintf("TOTAL : %d, FREE: %d\n", total_free, nfree);
		php0 = page_alloc(ALLOC_HUGE);
		//php1 = page_alloc(ALLOC_HUGE);
		page_free(php0);
		//page_free(php1);
		for (pp = page_free_list, nfree = 0; pp; pp = pp->pp_link)
				++nfree;
		cprintf("AFTER TOTAL : %d, FREE: %d\n", total_free, nfree);
    pp0 = pp1 = php0 = 0;
    assert((pp0 = page_alloc(0)));
